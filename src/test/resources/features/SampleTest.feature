Feature: Test
  ...
  Scenario:And Click on "clickable::Get Started Now" and wait 1 seconds
    And Navigate to url "sampleSite"
    And Click on "Link::Log in / Sign up" inside "ByClass::__actions"
    And Set "ByPlaceholder::you@example.com" value "{{username}}" using "fillText" handler
  ...
  Scenario:And Click on "clickable::Get Started Now" and wait 1 seconds
    ...
    And Fill the values for fields
      | Field                           | Value                  | Handler      |
      | $ByPlaceholder::you@example.com | {{username}}           | ((fillText)) |
      | $ById::password                 | =decrypt({{password}}) | ((fillText)) |
    And Click on "TextButton::Sign in"
    And Waiting for "ByClass::profile"
    And Wait 3 seconds
